...
export async function updateSession(request: NextRequest) {
  ...
  const isPublicRoute =
    ...
    // /cms/* — Decap CMS static assets (index.html, config.yml).
    ...
    // middleware redirects /cms/config.yml → /login and the CMS
    // boots with "Failed to load config.yml (404)".
    //
    // The path is /cms NOT /admin — /admin/* is the existing
    // super-admin portal (src/app/(app)/admin/*) and must keep its
    // requireSuperAdmin() gate intact.
